...
% 这一段是需要修改的
% CDABSLP 算法通过固定标签更新过程中节点的顺序，并且改进节点标签更新时标签选择的方法来提高 LPA 算法的性能。首先，算法计算网络中每个节点的影响值作为节点重要性的评判指标，并按照节点影响值降序排列作为标签更新过程中节点的顺序；然后，算法迭代的执行标签传播过程，直到检测到网络的社区结构。在每次标签传播过程中，CDABSLP 将节点影响值引入到标签计算公式中构造新的标签计算方法，计算邻接点中出现的每一个标签的重要性，更新节点标签。满足终止条件后，算法根据节点的标签将其划分到相应社区中，得到最终的社区划分结果。
...
由此出发，本章提出了一种全新的节点影响力模型。该模型在关注节点自身k-核值的同时，也考虑到了节点邻居的k-核值。节点i的影响力$NI(i)$的计算方式可参考公式\ref{eqn:NI}。
...
按照节点影响力模型计算每个节点的影响力，把所有节点按照节点影响力降序排列，以此顺序作为节点更新标签时的顺序可以使算法更加稳定。
...
基于以上多项稳定性设计原则，本章提出的CDABSLP算法的在整体上可以分为初始化阶段、标签传播阶段和社区划分阶段三大部分，图\ref{fig:cdabslp}为CDABSLP算法的整体流程图。
...
\begin{figure}
 ...
 在算法的初始化阶段中：（1）每个节点被设置一个唯一的标签，即为其节点ID，在图\ref{fig:CDABSLPshili}中节点内的数字即为其初始化的标签，节点$\{ v_1,v_2,v_3,v_4,v_5,v_6 \} $的标签分别被初始化为$\{ 1,2,3,4,5,6 \} $；（2）对网络进行k-核分解，获取所有节点的k-核值$Ks(i)$，并在计算时，将中间过程中统计得出的每个节点的度保留下来以备下一步使用；（3）通过节点影响力公式\ref{eqn:NI}在初始化阶段计算每个节点的节点影响力，在图\ref{fig:CDABSLPshili}中每个节点外的浮点数值即代表其节点影响力大小，节点$\{ v_1,v_2,v_3,v_4,v_5,v_6 \} $的节点影响力分别为$\{ 4.333,3.667,4.333,4.333,3.667,4.333 \} $，此处小数点后仅保留了3位；（4）将所有节点以节点影响力大小的降序进行排列后加入标签更新队列，节点影响力相同的节点，按照节点ID号升序排列，图\ref{fig:CDABSLPshili}所示的网络中，此排列即是按照$v_1-v_3-v_4-v_6-v_2-v_5$的顺序排列。CDABSLP算法初始化阶段具体可参考算法\ref{alg:CDABSLPchushihua}。
...
% 节点更新的顺序，标签更新过程如图\ref{fig:CDABSLPshili}所示。
